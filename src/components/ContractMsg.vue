    ...
    <div class="hd-section">
      ...
      <div class="info">
        ...
        <div v-if="contract.proxyAddress" class="info-item info-copy" @click="toEtherscanAddress(contract.proxyAddress, contract.chain)">
          ...
          <div class="info-item-value info-item-value-a flex-center-sb"><span>{{formatAddr(contract.proxyAddress)}} <label v-if="contract.verified" :style="{background: contract.verified ? '#375cff' :  '#F43658'}">{{contract.verified ? 'verified' : 'not verified'}}</label></span>
